...
class CheckBox(QWidget):
    ...
    def initGui(self):
      ...
      self.action = QAction(QIcon(icon), 'Do basic actions on the attribute table', self.iface.mainWindow()) #written when hovered
      self.action.triggered.connect(self.run) #runs run() when clicked
      self.iface.addPluginToMenu('&Basic actions', self.action)
      self.iface.addToolBarIcon(self.action)

    def unload(self):
      self.iface.removeToolBarIcon(self.action)       
      self.iface.removePluginMenu('&Basic actions', self.action)  
      del self.action  
    ...
    def change_active(self, on):
        if on:
            ...
            item, ok = QInputDialog.getItem(self, "select a name to change", 
         "list of fields", items, 0, False) # get field to delete with selector
            if ok and item:
    ...
    def delete_active(self, on):
        if on:
            ...
            prov = layer.dataProvider()
            items = ([field.name() for field in prov.fields()])
            item, ok = QInputDialog.getItem(self, "select a field to delete", 
         "list of fields", items, 0, False) # get field to delete with selector
            if ok and item:
    ...
    def copy_active(self, on):
        if on :
            ...
            prov = layer.dataProvider()
            items = ([field.name() for field in prov.fields()])
            field_to_copy, ok = QInputDialog.getItem(self, "select a field to delete", 
         "Copier les valeurs de...", items, 0, False) # get field to delete with selector
            if ok and field_to_copy:
                items = ([field.name() for field in prov.fields()])
                field_to_paste, ok = QInputDialog.getItem(self, "select a field to delete", 
...
def copy_column(field_to_copy, field_to_paste):
    ...
    #field_to_copy = getTextInput("Copier les valeurs d'une colonne", 'Veuillez entrer le nom de la colonne dont vous voulez copier les valeurs')
    #field_to_paste = getTextInput("Copier les valeurs d'une colonne", 'Veuillez entrer le nom de la colonne dans laquelle copier les valeurs')
